                        ...
                        <tr>
                            ...
                            <td>
                                @if (auth()->user()->id == 15)
                                    ...
                                    <span
                                        class="badge 
        @if ($req->status_admin == 'approved') bg-success
        @elseif($req->status_admin == 'rejected') bg-danger
        @else bg-secondary @endif">
                                        {{ $req->status_admin == 'approved' ? 'Diterima' : ($req->status_admin == 'rejected' ? 'Ditolak' : 'Pending') }}
                                    </span>
            ...
            document.querySelectorAll('.btn-history').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    fetch(`/superadmin/history/${tiket}/api`)
                        ...
                        .then(data => {
                            // Isi data request
                            document.getElementById('modal-tiket-display').textContent = data
                                .permintaan.tiket;
                            document.getElementById('modal-requester-display').textContent =
                                data.permintaan.user?.name || '-';
                            document.getElementById('modal-tanggal-request-display')
                                .textContent = new Date(data.permintaan.tanggal_permintaan)
                            ...
                            // Isi data pengiriman
                            if (data.pengiriman) {
                                document.getElementById('modal-tanggal-pengiriman-display')
                                    .textContent = new Date(data.pengiriman.tanggal_transaksi)
                                    .toLocaleDateString('id-ID', {
                                        year: 'numeric',
                                        month: 'long',
                                        day: 'numeric'
                                ...
                                const pengirimanTable = document.getElementById(
