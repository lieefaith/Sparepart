...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const fileInput = safeQuery('#fileUpload') || safeQuery('input[type="file"][name="file_upload"]') || null;
    ...
    // ----- Function to fill modal with fetched data -----
    async function loadRequestDetailToModal(tiket) {
      ...
      try {
        const url = `/requestbarang/${encodeURIComponent(tiket)}`;
        const resp = await fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const data = await resp.json();

        // Basic request info
        safeQuery('#modal-tiket-display') && (safeQuery('#modal-tiket-display').textContent = data?.tiket ?? tiket);
        safeQuery('#modal-requester-display') && (safeQuery('#modal-requester-display').textContent = data?.name ?? 'User');
        if (data?.tanggal_permintaan) {
          safeQuery('#modal-tanggal-request-display') && (safeQuery('#modal-tanggal-request-display').textContent = formatDateId(data.tanggal_permintaan));
        }

        // Populate request details table
        if (requestTableBody) {
        ...
        // Populate pengiriman details
        if (data?.pengiriman) {
          if (data.pengiriman.tanggal_transaksi) {
            safeQuery('#modal-tanggal-pengiriman-display') && (safeQuery('#modal-tanggal-pengiriman-display').textContent = formatDateId(data.pengiriman.tanggal_transaksi));
          }
          if (pengirimanTableBody) {
            pengirimanTableBody.innerHTML = '';
            const pDetails = Array.isArray(data.pengiriman.details) ? data.pengiriman.details : [];
    ...
    btnKonfirmasi && btnKonfirmasi.addEventListener('click', async function () {
      ...
      const tiketVal = (inputTiket && inputTiket.value) || (safeQuery('#modal-tiket-display') && safeQuery('#modal-tiket-display').textContent) || '';
      if (!tiketVal) {
      ...
      const noResiEls = safeQueryAll('input[name="no_resi"]'); // there may be duplicates -> prefer visible non-empty
    ...
    window.showStatusDetailModal = async function (tiket, userRole = 'user') {
      if (!tiket) return;
      try {
        const resp = await fetch(`/user/validasi/${encodeURIComponent(tiket)}/api`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const data = await resp.json();
    ...
    (function handleFormEkspedisiToggle() {
      ...
      if (!toggle) {
        ...
        const anyValue = safeQueryAll('#formEkspedisi input, #formEkspedisi select, #formEkspedisi textarea').some(i => i.value && i.value.trim() !== '');
